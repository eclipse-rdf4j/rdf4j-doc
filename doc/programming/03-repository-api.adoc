...
However, it is very important to note that `RepositoryConnection` is *not* guaranteedto be thread-safe. This means that you should not try to share a single `RepositoryConnection` over multiple threads. Instead, ensure that each thread creates and uses its own `RepositoryConnection`s, and use transaction isolation levels to control visibility of concurrent updates between threads.
